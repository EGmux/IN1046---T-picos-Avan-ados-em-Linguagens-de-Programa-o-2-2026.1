...
Reconhecimento da relação com princípios clássicos de engenharia de *software* — muitos dos princípios identificados (modularização, separação de responsabilidades, camadas de abstração) não são novos, mas sua aplicação sistemática a *feature models* é uma contribuição relevante, pois valida que boas práticas conhecidas se estendem a esse domínio específico.
