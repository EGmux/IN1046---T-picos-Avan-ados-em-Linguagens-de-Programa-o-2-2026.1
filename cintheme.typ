...
#let slide(
 ...
 let footer(self) = {
    ...
    set text(size: .4em,fill: rgb("#000000"), weight: "bold")
    {
      ...
      grid(
        ...
        rows: 1.5em,
        cell(fill: rgb("#FFFFFF"), utils.call-or-display(
          self,
          self.store.footer-a,
        )),
        cell(fill: rgb("#FFFFFF") , utils.call-or-display(
...
#let title-slide(
  ...
  let body = {
    if info.logo != none {
      place(right, text(fill: rgb("#FFFFFF"), info.logo))
    }
    std.align(
       left + horizon,
      {
        block(
          ...
          breakable: false,
          {
            text(size: 2em, fill: rgb("#FFFFFF") , info.title)
            if info.subtitle != none {
              parbreak()
              text(size: 1.2em, fill: rgb("#FFFFFF"), info.subtitle)
            }
          },
        )
        set text(size: .8em)
        ...
        if info.institution != none {
          parbreak()
          text(size: .9em, info.institution, fill: rgb("#FFFFFF"))
        }
        if info.date != none {
          parbreak()
          text(size: .8em, utils.display-info-date(self), fill: rgb("#FFFFFF"))
        }
      },
    )
  }
...
#let new-section-slide(
  ...
  let slide-body = {
    ...
    show: pad.with(20%)
    set text(size: 1.5em, fill: rgb("#000000"), weight: "bold")
    stack(
      dir: ttb,
      spacing: .65em,
...
#let cintheme(
  ...
  align: top,
  progress-bar: true,
  header: self => self.info.placeholder,
  header-right: self => (
       h(.3em) + self.info.logoalternative
  ),
  footer-columns: (25%, 1fr, 25%),
  footer-a: self => self.info.placeholder,
